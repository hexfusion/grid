...
async fn a_provider_enrolls_and_collects_a_certificate() {
    ...
    let (create_status, created) = call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", &plain_csr()))).await;
    ...
    let (approve_status, approved) =
        call_as_operator(&app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;
    ...
    let (fetch_status, fetched) = call(&app, "GET", &format!("/v1alpha1/requests/{id}"), None).await;
...
async fn the_certificate_never_carries_a_name_the_request_asked_for() {
    ...
    let (_create_status, created) = call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", &csr))).await;
    let id = created["requestId"].as_str().expect("id").to_owned();
    let (_approve_status, approved) =
        call_as_operator(&app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;
...
async fn approving_twice_issues_one_certificate() {
    let app = service();
    let (_create_status, created) =
        call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", &plain_csr()))).await;
    let id = created["requestId"].as_str().expect("id").to_owned();

    let (first_status, first) = call_as_operator(&app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;
...
async fn deciding_requires_an_operator_credential() {
    ...
    let (approve_status, approve_body) = call(&app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;
    assert_eq!(
        approve_status,
    ...
    let (deny_status, _deny_body) = call(&app, "POST", &format!("/v1alpha1/requests/{id}/deny"), None).await;
    assert_eq!(deny_status, StatusCode::UNAUTHORIZED, "denying must be refused too");

    let (list_status, _list_body) = call(&app, "GET", "/v1alpha1/requests", None).await;
    assert_eq!(
        list_status,
        StatusCode::UNAUTHORIZED,
    ...
    let (_fetch_status, fetched) = call(&app, "GET", &format!("/v1alpha1/requests/{id}"), None).await;
...
async fn an_unknown_token_decides_nothing() {
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/v1alpha1/requests/{id}/approve"))
...
async fn listing_can_be_filtered_by_phase() {
    let app = service();
    let (_first_status, first) = call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", &plain_csr()))).await;
    let id = first["requestId"].as_str().expect("id").to_owned();
    call(&app, "POST", "/v1alpha1/requests", Some(submit("site-e", &plain_csr()))).await;
    call_as_operator(&app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;

    let (_pending_status, pending) = call_as_operator(&app, "GET", "/v1alpha1/requests?phase=pending", None).await;
    ...
    let (_issued_status, issued) = call_as_operator(&app, "GET", "/v1alpha1/requests?phase=issued", None).await;
    assert_eq!(
...
async fn a_denied_request_issues_nothing() {
    let app = service();
    let (_status, created) = call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", &plain_csr()))).await;
    let id = created["requestId"].as_str().expect("id").to_owned();

    let (deny_status, denied) = call_as_operator(
        &app,
        "POST",
        &format!("/v1alpha1/requests/{id}/deny"),
...
async fn an_unusable_request_is_refused_on_submission() {
    ...
    let (malformed_status, malformed_body) =
        call(&app, "POST", "/v1alpha1/requests", Some(submit("site-d", "not a csr"))).await;
    assert_eq!(
        malformed_status,
    ...
    let (list_status, listed) = call_as_operator(&app, "GET", "/v1alpha1/requests", None).await;
...
async fn an_unknown_request_is_not_found() {
    ...
    let (status, body) = call(&app, "GET", &format!("/v1alpha1/requests/{missing}"), None).await;
...
async fn requests_are_listed_newest_first() {
    ...
    for site in ["site-d", "site-e", "site-f"] {
        call(&app, "POST", "/v1alpha1/requests", Some(submit(site, &plain_csr()))).await;
    }

    let (_list_status, listed) = call_as_operator(&app, "GET", "/v1alpha1/requests", None).await;
...
async fn an_approved_provider_collects_what_it_needs_to_join() {
    ...
    let (status, kit) = call(
        &app,
        "POST",
        &format!("/v1alpha1/requests/{id}/join"),
...
async fn a_pending_request_has_no_joining_kit() {
    ...
    let (_status, created) = call(&app, "POST", "/v1alpha1/requests", Some(submit("site-early", &csr))).await;
    let id = created["requestId"].as_str().expect("id").to_owned();
...
async fn approved_site(app: &axum::Router, site: &str) -> (String, KeyPair) {
    ...
    let (_status, created) = call(app, "POST", "/v1alpha1/requests", Some(submit(site, &csr))).await;
    let id = created["requestId"].as_str().expect("id").to_owned();
    call_as_operator(app, "POST", &format!("/v1alpha1/requests/{id}/approve"), None).await;
    (id, key)
}
