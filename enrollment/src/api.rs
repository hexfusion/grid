//! The HTTP interface, implementing `api/enrollment-v1alpha1.yaml`.
//!
...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        .route("/v1alpha1/requests", post(create).get(list))
        .route("/v1alpha1/requests/{request_id}", get(fetch).delete(remove))
        .route("/v1alpha1/requests/{request_id}/approve", post(approve))
        .route("/v1alpha1/requests/{request_id}/deny", post(deny))
        .route("/v1alpha1/requests/{request_id}/join", post(join))
